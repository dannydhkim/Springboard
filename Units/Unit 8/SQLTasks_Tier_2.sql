...
FROM Members
WHERE memid = (SELECT MAX(memid)-1
		FROM Members) 
OR memid = (SELECT MIN(memid)+1
		FROM Members)
...
the member name. */
SELECT DISTINCT CONCAT(m.surname, ', ', m.firstname,' USED ', f.name) AS 'Tennis Court Users'
...
SELECT name, CONCAT(firstname, ', ', surname), CURDATE(starttime),
	CASE WHEN membercost > guestcost THEN membercost
		WHEN guestcost > membercost THEN guestcost
		END AS cost
FROM Bookings AS b
...
SELECT name, CONCAT(firstname, ', ', surname), starttime,
	CASE WHEN membercost > guestcost THEN membercost
		WHEN guestcost > membercost THEN guestcost
		END AS cost
FROM 
	(SELECT *
     	FROM Bookings
		WHERE starttime > '2012-09-14 00:00:00' 
		AND starttime < '2012-09-14 23:59:59') AS b
INNER JOIN Facilities AS f ON b.facid = f.facid
INNER JOIN Members AS m ON b.memid = m.memid
